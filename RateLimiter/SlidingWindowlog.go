...
func NewSlidingLogLimiter(smallWindow time.Duration, strategies ...*SlidingLogLimiterStrategy) (*SlidingLogLimiter, error) {
	...
	// 不能不设置策略
	if len(strategies) == 0 {
		return nil, errors.New("strategies must not be empty")
	}
...
func (l *SlidingLogLimiter) TryAcquire() (bool, error) {
	l.mu.Lock()
	defer l.mu.Unlock()

	// 获取当前小窗口值
	currentWindow := time.Now().UnixNano() / l.smallWindow * l.smallWindow
	// 获取每个策略的起始小窗口值
	startSmallWindows := make([]int64, len(l.strategies))
	for i, strategy := range l.strategies {
		startSmallWindows[i] = currentWindow - l.smallWindow*(strategy.smallWindows-1)
	}
	...
	for i, strategy := range l.strategies {
		if counts[i] >= strategy.limit {
			return false, &ViolationStrategyError{
